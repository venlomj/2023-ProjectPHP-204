<div>
    ...
    <x-kmad.section
        ...
        <div class="p-4 flex justify-between items-start gap-4">
            <div class="relative w-64">
                <x-button wire:click="setNewDistance()">
                    Nieuwe Afstand
                </x-button>
        ...
        <x-input-error for="newDistance" class="m-4 -mt-4 w-full"/>
        <div
            x-show="open"
            style="display: none"
            ...
            <x-kmad.list type="ul" class="list-outside mx-4 text-sm">
                <li>
                    Op deze pagina kan de admin de afstanden beheren.
                </li>
    ...
    <div style="overflow-x:scroll">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <div class="my-4">{{ $distances->links() }}</div>
            @foreach ($distances as $distance)
                <tr wire:key="$distance{{ $distance->id }}">
                    <td class="px-6 py-4 whitespace-nowrap">{{ $distance->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <x-phosphor-pencil-line-duotone class="outline-0"
                                                        data-tippy-content="Aanpassen" wire:click="setNewDistance({{ $distance->id }})"
                        ...
                        <x-phosphor-trash-duotone class="outline-0"
                                                  data-tippy-content="Verwijderen" x-data=""
                                                   @click="confirm('Bent u zeker dat u de afstand wilt verwijderen?') ? $wire.deleteDistance({{ $distance->id }}) : ''"
    ...
    <x-dialog-modal id="distanceModal"
                    wire:model="showModal">
        <x-slot name="title">
            <h2>{{ is_null($newDistance['id']) ? 'Nieuwe afstand' : 'Aanpassen afstand' }}</h2>
        </x-slot>
            ...
            <div class="flex flex-row gap-4 mt-4">
                <div class="flex-1 flex flex-col gap-2">
                    ...
                    <x-input id="name" type="string"
                             wire:model.defer="newDistance.name"
        ...
        <x-slot name="footer">
            ...
            @if(is_null($newDistance['id']))
                <x-button
                    wire:click="createDistance()"
            ...
            @else
                <x-button
                    color="success"
                    wire:click="updateDistance({{ $newDistance['id'] }})"
                    wire:loading.attr="disabled"
                    class="ml-2">Update
                </x-button>
            @endif
